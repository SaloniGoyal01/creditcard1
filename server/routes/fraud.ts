...
export const blockTransaction: RequestHandler = (req, res) => {
  ...
  console.log(`Transaction ${transactionId} blocked by user. Reason: ${reason}`);
...
export const getFraudAnalytics: RequestHandler = (req, res) => {
  ...
  // Geographic analysis
  const locationCounts = transactions.reduce((acc, t) => {
  ...
  res.json({
    ...
    analytics: {
      ...
      riskDistribution: {
        ...
        medium: transactions.filter(t => t.riskScore >= 40 && t.riskScore < 80).length,
...
export const simulateTransaction: RequestHandler = (req, res) => {
  const newTransaction: Transaction = {
    ...
    merchant: ["Amazon", "Flipkart", "PayTM", "Google Pay", "Unknown Merchant", "Electronics World"][Math.floor(Math.random() * 6)],
    location: ["Mumbai", "Delhi", "Bangalore", "Chennai", "Moscow", "London"][Math.floor(Math.random() * 6)] + ", " + (Math.random() > 0.7 ? "India" : ["Russia", "UK", "USA"][Math.floor(Math.random() * 3)]),
...
function sendFraudAlert(email: string, transaction: Transaction, reason: string) {
