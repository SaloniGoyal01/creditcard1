...
export const addToBlockchain: RequestHandler = (req, res) => {
  const { transactionId, action, amount, merchant, location, riskScore, reason, userAction } = req.body;
...
export const getBlockchainStats: RequestHandler = (req, res) => {
  const stats = {
    ...
    actionBreakdown: blockchain.reduce((acc, block) => {
...
export const exportBlockchain: RequestHandler = (req, res) => {
  const exportData = {
    ...
    integrity: {
      ...
      validHashes: blockchain.filter(b => b.blockHash.startsWith("0000") || b.id === "genesis").length,
      lastValidation: new Date().toISOString()
    }
  };
  
  res.setHeader('Content-Type', 'application/json');
  res.setHeader('Content-Disposition', 'attachment; filename=blockchain_export.json');
  res.json(exportData);
};
