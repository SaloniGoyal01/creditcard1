...
export default function Login() {
  ...
  const [authMethod, setAuthMethod] = useState<"email" | "voice" | "biometric">(
    "email",
  );
        ...
        <Card className="w-full max-w-md border-neon-green neon-border">
          <CardContent className="flex flex-col items-center justify-center p-8">
            ...
            <div className="w-full bg-muted rounded-full h-2">
              <div
                className="bg-neon-green h-2 rounded-full animate-pulse"
                style={{ width: "100%" }}
              ></div>
            </div>
          </CardContent>
              ...
              >
                ...
                <TabsContent value="voice" className="space-y-4">
                  <div className="text-center space-y-4">
                    <div className="flex items-center justify-center">
                      <div
                        className={`p-8 border-2 border-dashed rounded-full ${voiceStatus === "recording" ? "border-neon-cyan animate-pulse" : "border-muted"}`}
                      >
                        {voiceStatus === "recording" ? (
                          <Mic
                            className={`h-16 w-16 ${getVoiceStatusColor()} neon-glow`}
                          />
                        ) : voiceStatus === "processing" ? (
                          <Loader2
                            className={`h-16 w-16 ${getVoiceStatusColor()} animate-spin`}
                          />
                        ) : voiceStatus === "success" ? (
                          <CheckCircle
                            className={`h-16 w-16 ${getVoiceStatusColor()} neon-glow`}
                          ...
                          <XCircle
                            className={`h-16 w-16 ${getVoiceStatusColor()}`}
                          />
                        ) : (
                ...
                <TabsContent value="biometric" className="space-y-4">
                  <div className="text-center space-y-4">
                    <div className="flex items-center justify-center">
                      <div
                        className={`p-8 border-2 border-dashed rounded-full ${biometricStatus === "scanning" ? "border-neon-purple animate-pulse" : "border-muted"}`}
                      >
                        <Fingerprint
                          className={`h-16 w-16 ${getBiometricStatusColor()} ${biometricStatus === "scanning" ? "neon-glow" : ""}`}
                        />
                      </div>
                    </div>
